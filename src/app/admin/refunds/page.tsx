...
export default function AdminRefundsPage() {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ['admin-refunds', { page, search: debouncedSearch, status: statusFilter, method: methodFilter }],
    queryFn: async () => {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: 'Total Refunds', value: meta.total || 0, color: 'text-gray-900' },
          { 
            label: 'Pending', 
            value: refunds.filter((r: any) => r.refundStatus === 'PENDING').length,
            color: 'text-yellow-600' 
          },
          { 
            label: 'Processing', 
            value: refunds.filter((r: any) => r.refundStatus === 'PROCESSING').length,
            color: 'text-blue-600' 
          },
          { 
            label: 'Completed', 
            value: refunds.filter((r: any) => r.refundStatus === 'COMPLETED' || r.refundStatus === 'PROCESSED').length,
